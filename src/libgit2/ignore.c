...
static int get_internal_ignores(git_attr_file **out, git_repository *repo)
{
	...
	/* parse the default rules as part of loading the in-memory file, so
	 * that it's fully built before the cache publishes it to other threads */
	...
	const char *pathname)
{
	...
	while (1) {
		int found = GIT_IGNORE_NOTFOUND;

		/* first process builtins - success means path was found */
		if (ignore_lookup_in_rules(&found, ignores.ign_internal, &path))
			goto found;

		/* next process files in the path */
		git_vector_foreach(&ignores.ign_path, i, file) {
			if (ignore_lookup_in_rules(&found, file, &path))
				goto found;
		}
...
found:
		if (found == GIT_IGNORE_TRUE) {
			*ignored = 1;
			goto cleanup;
		}

		/* Either nothing matched, or a negation did. Git never re-includes
		 * a path under an excluded directory, so in both cases the parents
		 * still get a say. */
